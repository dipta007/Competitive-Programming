        ...
        FOR(i,0,(int)vp.size()-1)
        {
            FOR(j,0,7)
            {
                ...
                if(u>0 && v>0 && u<=n && v<=m && u!=x && v!=y)
                {
                    ...
                    mcmf::addEdge(mp[p], mp[vp[i]], 1, 0);
                    if(B[p]==0)
                    {
                        B[p]=1;
                        mcmf::addEdge(src, mp[p], INF, 1);
                    }

//                    mcmf::addEdge(F[p], mp[vp[i]], 1, 0);
//                    mcmf::addEdge(B[p], F[p], INF, 1);
//                    mcmf::addEdge(src, B[p], INF, 0)
                }
            }
        }
        ...
        {
            pii p = vp[i];
            mcmf::addEdge(mp[p], sink, 1, 0);
        }
        mcmf::init(src, sink, in);
