...
int main() {
    ...
    FOR(ci,1,t)
    {
        ...
        poly.getline();
        line l = line(point(x, -1e9-7), point(x, 1e9+7));
        vector < point > vp;
        FOR(i,0,poly.n-1)
        {
            if(poly.l[i].segmentIntersection(l))
            {
                point p = poly.l[i].crosspoint(l);
                vp.PB(p);
            }
        }
        sort(ALL(vp));
        ...
        {
            res += vp[i].distance(vp[i-1]);
            trace(vp[i].x, vp[i].y, vp[i-1].x, vp[i-1].y);
        }
        trace(vp.size(), res);
        if(res > thresh + EPS || fabs(res-thresh) < EPS) cout << "YES" << endl;
        else cout << "NO" << endl;
    }
