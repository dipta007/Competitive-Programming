...
//       WRITE("out.txt");
    ...
    FOR(ci,1,t)
    {
        int n, m;
        cin >> n >> m;
        FOR(i,0,n-1) cin >> CR[i];
        FOR(i,0,m-1) cin >> CC[i];

        FOR(i,1,n-1) R[i] = CR[i] - CR[i-1];
        FOR(i,1,m-1) C[i] = CC[i] - CC[i-1];

        C[0] = CC[0];
        R[0] = CR[0];

        flow::init(n+m+7, 0, n+m+1);
        FOR(i,0,n-1)
        {
            trace(R[i]);
            flow::addEdge(0, i+1, R[i] - m);
        }
        FOR(i,0,m-1)
        {
            trace(C[i]);
            flow::addEdge(n+i+1, n+m+1, C[i] - n);
        }
