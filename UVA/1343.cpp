...
inline unsigned int queryy(int x, int y)
{
    if(x == y-1) return 0;
    int x1 = x, y1 = x, x2 = y, y2 = y;
    return query(x2-1,y2-1) + query(x1, y1) - query(x2-1, y1) - query(x1, y2-1);
}

unsigned int n, arr[504];
...
struct data
{
    ...
    unsigned int val;
    data () {}
    data(int _x, int _y, unsigned int _v)
    {
        x = _x, y = _y, val = _v;
    }
};
...
int main() {
    ...
    FOR(ci,1,t)
    {
        ...
        FOR(i,1,n)
        {
            FOR(j,i+1, n)
            {
                vd.PB(data(i, j, arr[i] + arr[j]));
            }
        }

        sort(ALL(vd), cmp);

//        fenwick2D::init(n*n+4);
        ...
        {
            data d = vd[i];
            unsigned int now = queryy(d.x, d.y);
            now ++;

            trace(i, d.x, d.y, d.val, res, now);
            update(d.x, d.y, now);
