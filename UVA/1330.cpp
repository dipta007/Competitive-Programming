...
{
    flow::init(n+m+4, 0, n+m+2);
    FOR(i,1,n) flow::addEdge(0, i, row[i]);
    FOR(i,1,m) flow::addEdge(n+i, n+m+2, col[i]);
    FOR(i,1,n)
    {
        FOR(j,1,m)
        {
...
void revertGraph(int n, int m, int x, int y)
{
    flow::flow[0][x]++;
    flow::flow[n+y][n+m+2]++;
...
void updateGraph(int n, int m, int x, int y)
{
    flow::flow[0][x]--;
    flow::flow[n+y][n+m+2]--;
...
ll check(int n, int m, int x, int y, ll sum)
{
    updateGraph(n, m, x, y);
    int kk = flow::check(sum);
    if(!kk) revertGraph(n, m, x, y);
    return kk;
}
...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    FOR(ci,1,t)
    {
        int n, m;
        getII(n,m);
        FOR(i,1,n) getI(row[i]);
        FOR(i,1,m) getI(col[i]);

        int sumr = 0, sumc = 0;
        FOR(i,1,n) sumr += row[i];
        FOR(i,1,m) sumc += col[i];

        int flg = (sumr == sumc) && (init(n, m) == sumr);
        ...
        if(flg)
        {
            FOR(i,1,n)
            {
                FOR(j,1,m)
                {
                    if(flow::flow[i][n+j] == 0)
                    {
                        flow::cap[i][n+j] = 0;
                        continue;
                    }
                    int now = check(n, m, i, j, sumr);
                }
            }
        }
