...
int turn( point O , point A , point B) { //
    ...
    if( res+EPS < 0 ) return -1 ; // O->A->B is a right turn
...
inline bool comp(const point &a, const point &b) {
    ...
    if(fabs(d) > EPS && sqDist(P0, a) > sqDist(P0, b)) return false;
    return true;
}
...
int main() {
    ...
    while(cin >> n)
    {
        ...
        convexHull(n);
        reverse(P, P+n);
        FOR(i,0,n-1) poly.add(P[i]);

        double x, y;
        cin >> x >> y;
        if(poly.containsInPoly(point(x,y), true) == true) cout << "T" << endl;
        else cout << "F" << endl;
    }
