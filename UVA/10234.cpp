...
int main() {
    ...
    while(getline(cin, st))
    {
        text = st;
        int len = st.size();
        FOR(i,0,len-1) if(isupper(text[i])) text[i] = tolower(text[i]);
        suffixArray();
        ...
        FOR(i,1,q)
        {
            ...
            {
                debug(j, LCP[j], res, in, SA[j])
                if(LCP[j] >= nn)
                {
                    int cnt = 1, k=j;
                    while(k < len && LCP[k] >= nn) cnt++, k++;
            ...
            cout << res << " " << text.substr(in, nn) << endl;
        }
        cin.get();
    }
