...
//       WRITE("out.txt");
    ...
    while( cin >> m )
    {
        ...
        FOR(i,0,len-1)
        {
            debug("dd", i, SA[i], LCP[i])
            if(LCP[i] >= ans)
            {
                int cnt = 1, j=i;
                int mx = 0;
                mx = max(mx, SA[j]);
                while(j <= len && LCP[j] >= ans) j++, cnt++, mx = max(mx, SA[j]);

                debug(cnt, in, mx)
                if(cnt >= m) in = max(in, mx);
...
//        debug(s.size())
//        if(ans == 1) in = text.size()-1;
        if(ans==0) cout << "none" << endl;
        else cout << ans << " " << in << endl;
    }
...
//b a a a a b a b a b a  b  b  a  b  a  b  b  a  b
