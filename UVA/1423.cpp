...
unsigned long long fnv_hash(unsigned int *p, int len)
{
...
inline unsigned long long hashing(unsigned int *p, int len)
{
    return fnv_hash(p, len);
}

map <unsigned ll, int> ump;
unsigned int tmpR[30+4];
unsigned int tmpP[30+4];
...
int main() {
    ...
    FOR(ci,1,t)
    {
        ...
        FOR(i,0,k-1) tmpR[i] = tmpP[i] = 0;
        unsigned long long now = hashing(tmpR, k);
        ump[ now ] = 1;
...
//        trace(now, ump[now]);
        ...
        while(i < vp.size())
        {
            ...
            if(ump[now] > 0) {
                if(mx) A = max(A, (ti-1) - (ump[now]-1));
            }
            while(i<vp.size() && vp[i].ff == ti)
            {
                tmpP[vp[i].ss]++;
                mx = max(mx, tmpP[vp[i].ss]);
                i++;
            }
            FOR(j,0,k-1)
            {
                tmpR[j] = mx - tmpP[j];
//                trace(j, tmpP[j], tmpR[j]);
            }

            now = hashing(tmpR, k);
//            trace(now, ump[now], i, ti);
            if(ump[now] == 0) ump[now] = ti+1;
        }

        A = max(A, l - (ump[now] - 1));

//        int now = hashing(tmpR, k);
//        mx = max(mx, l - (ump[now] - 1) + 1);

        cout << "Case " << ci << ": " << A << " meter(s)" << endl;
    }
