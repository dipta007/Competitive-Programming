...
int check(int l, int r, int x, int y)
{
    ...
    while(low <= high)
    {
        ...
        ll fst = helper.getForwardHashingQuery(l,l+mid);
        ll scnd = helper.getForwardHashingQuery(x,x+mid);
    ...
    if(st[l+res]<st[x+res]) return 1;
    else if(st[l+res]>st[x+res]) return 2;
    else
    {
        if(l<x) return 1;
        else return 2;
...
int main() {
    ...
    FOR(ci,1,t)
    {
        ...
        FOR(i,0,len-1)
        {
            debug("i",i, helper.getForwardHashingQuery(4,6), helper.getForwardHashingQuery(0,2));
            int kk = check(low, low+len-1, i, i+len-1);
            debug(kk)
            if(kk==2) low = i;
        }
        cout << low+1 << endl;
