...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    FOR(ci,1,t)
    {
        ...
        while(low <= high)
        {
            ...
            FOR(j, 0, len-mid)
            {
                ll fhash = helper.getForwardHashingQuery(j, j+mid-1);
                mp[fhash]++;
                if(mp[fhash]>=2)
                {
                    flg=1;
                    val = fhash;
                    //debug(val)
                    string stt = st.substr(j, mid);
                    if(check(now, stt)==2) now = stt;
                }
            }
        ...
        if(resSt=="ZZZZ") cout << "No repetitions found!" << endl;
        else cout << resSt << " " << res << endl;
    }
