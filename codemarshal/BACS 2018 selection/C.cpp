...
bool check(ll lft, ll c, ll x, ll l)
{
    if(lft >= l) return true;
    if(lft + c >= l)
    {
        ll no = x/2 + 1;
        if(lft + no >= l) return true;
        return false;
    }
    return false;
}

ll get(ll x, ll l, ll n)
{
    ll a[404];
    a[0] = 0;
    a[1] = 1;
...
//        READ("inC.txt");
    ...
    FOR(ci,1,t)
    {
        ...
        if(x%2 == 0)
        {
            if(l >= x) cout << "0/1" << endl;
            else
            {
                ll win = x-l;
                ll gc = __gcd(win, psbl);
                win /= gc;
                psbl /= gc;
                cout << win << "/" << psbl << endl;
            }
        ...
        while(tmp)
        {
            ...
            if(range && x <= odd && check(lft, c, x, l)) win++;
//            trace(range, x, odd, check(lft, c, x, l));
