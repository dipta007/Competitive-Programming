...
int call(int in, int fac)
{
    if(in >= c) return 0;
    int &ret = dp[in][fac];
    if(ret != -1) return ret;

    ll now = s + totCookie[fac];

    ret = 1 + call(in+now, fac);
    int lim = (1 << n) - 1;
    for ( int sub = 0 ; sub = sub - fac & fac ; )
    {
        ll ncost = totCost[sub];
        int tmp = fac & (~sub);
...
int main() {
    ...
    while(cin >> n >> c >> s)
    {
        ...
        FOR(i,0,mask)
        {
            ...
            {
                if(bitCheck(i, j) == 0)
                    totCookie[i] += cookie[j];
                else totCost[i] += cost[j];
            }
        }
        cout << call(0, mask) << endl;
    }
