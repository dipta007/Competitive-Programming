...
bool checkMin(int d, int m, int y)
{
    if(y != mny) return y < mny;
    else
    {
        if(m != mnm) return m < mnm;
        else d < mnd;
    }
    return false;
}
bool checkMax(int d, int m, int y)
{
    if(y != mxy) return y > mxy;
    else
    {
        if(m != mxm) return m > mxm;
        else d > mxd;
    }
    return false;
}
