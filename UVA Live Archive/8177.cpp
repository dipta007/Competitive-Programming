...
{
    if(l > r) return 0;
    if(r-l+1 < L) return INF;
    if(r-l+1 >= L && r-l+1 <= R) return sum[r] - sum[l-1];
    ll &ret = dp[l][r];
//    if(ret != -1) return ret;
    ...
    FOR(i, l, r)
    {
        int rr = min(i+R-1, r);
        int left = call(l, i-1);
        int mid = sum[rr] - sum[i-1];
        int right = call(rr+1, r);
        ll now = 2 * left + 2 * right + mid;
        trace(l, r, i, rr, left, mid, right, now);
        ret = min(ret, now);
    }
    return ret;
}
