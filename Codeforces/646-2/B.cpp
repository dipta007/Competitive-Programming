...
int call(int i, int nw, int change) {
  if (change >= 2) return 99999;
  if (i >= st.size()) {
    return 0;
  }

  int &ret = dp[i][nw][change];
  if (ret != -1) return ret;

  ret = 99999;
  int dig = st[i] - '0';
  if (dig == nw) {
    ret = min(ret, call(i+1, nw, change));
    ret = min(ret, 1 + call(i+1, !nw, change + 1));
  } else {
    ret = min(ret, 1 + call(i+1, nw, change));
    ret = min(ret, call(i+1, !nw, change + 1));
  }
  return ret;
}
...
int main()
{
  ...
  FOR(ci, 1, t) {
    cin >> st;
    SET(dp);
    cout << min(call(1, st[0] -'0', 0), 1 + call(1, !(st[0] -'0'), 0)) << endl;
  }
  return 0;
}