...
char CRTBUFF[30000];
     ...
     #define trace(...) __f(__LINE__, #__VA_ARGS__, __VA_ARGS__)
        ...
        void __f(const int line, const char* name, Arg1&& arg1){
            cerr << name << " : " << arg1 << std::endl;
        }
        template <typename Arg1, typename... Args>
        void __f(const int line, const char* names, Arg1&& arg1, Args&&... args){
...
ordered_set lft[2], rgt[2];

ll ans = 0;
int ind = 1;

void add(int x, int y, int in)
{
    lft[in].insert(MP(x, ind++));
    rgt[in].insert(MP(y, ind++));

    in = in^1;

    int tot = lft[in].size();
    int bad1 = rgt[in].order_of_key(MP(x, -INF));
    int bad2 = lft[in].order_of_key(MP(y, INF));
    bad2 = tot - bad2;
...
//    trace(t, t);
    FOR(ci,1,t)
    {
        FOR(i,0,1) lft[i].clear(), rgt[i].clear();
        ans = 0;
        ...
        FOR(i,1,n)
        {
            int x, y;
            cin >> x >> y;
            lft[0].insert(MP(x, ind++));
            rgt[0].insert(MP(y, ind++));
        }
