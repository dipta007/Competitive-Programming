...
void pre()
{
    ...
    FOR(i,1,n)
    {
        FOR(j,1,m)
        {
            FOR(k,0,LIM)
            {
                if(k==1 && a[i][j] == k) trace(i, j);
                cum[i][j][k] = (a[i][j] <= k) + cum[i-1][j][k] + cum[i][j-1][k] - cum[i-1][j-1][k];
            }
        }
    }
}
...
{
    trace(l1, r1, l2, r2, k, cum[l2][r2][k]);
    return (cum[l2][r2][k] - cum[l2][r1-1][k] - cum[l1-1][r2][k] + cum[l1-1][r1-1][k]);
}
