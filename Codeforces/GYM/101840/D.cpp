...
ll mst()
{
	...
	while(eind > 0)
	{
	    trace(eind, e[eind].ind, e[eind].v.size()-1);
	    if(e[eind].ind >= (int)e[eind].v.size()-1)
        {
            eind--;
            continue;
        }
	    int nu = e[ eind ].v[ e[eind].ind ];
	    int nv = e[ eind ].v[ e[eind].ind+1 ];

		int u=find( nu );
		int v=find( nv );
//	    trace(nu, nv, u, v, count);
		if(u!=v)
		{
			pr[u]=v;
			count++;
...
//    #endif // dipta007
    ...
    FOR(ci,1,t)
    {
        int n;
        cin >> n;
        FOR(i,0,100000) e[i].ind = 0, e[i].v.clear();
        FOR(i,1,n)
        {
            int x;
            cin >> x;
            FOR(j,0,(int)_div[x].size()-1)
            {
                int d = _div[x][j];
                e[d].v.PB(i);
            }
