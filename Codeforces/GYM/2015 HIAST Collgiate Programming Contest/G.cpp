...
bool backk(int kk = 0)
{
    ...
    FOR(i,1,n)
    {
        if(mark[i] == 0)
        {
            ...
            int tmp[104];
            CLR(tmp);
            FOR(j,1,n) tmp[j] = mark[j];
            FOR(j,1,x)
            {
                if(x%j == 0)
                    mark[j] = 1;
            }
//            trace(i, kk, tmp[1], tmp[2], tmp[3]);
            if(backk(kk+1) == false) return true;
//            trace("Sd", i, kk, tmp[1], tmp[2], tmp[3]);
            FOR(j,1,n) mark[j] = tmp[j];
        }
    }
//    trace("sdg");
    return false;
