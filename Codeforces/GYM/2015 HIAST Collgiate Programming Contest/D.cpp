...
ll call(int in, int normal, int best)
{
    if(normal > m) return 0;
    if(in >= n) return (normal == m && best >= k);

    ll &ret = dp[in][normal][best];
    if(ret != -1) return ret;

    ret = 0;
    ret += call(in+1, normal, best);;
    ret %= mod;
    if(a[in] >= d)
    {
        ret += call(in+1, normal+1, best+1);
        ret %= mod;
    }
    else
    {
        ret += call(in+1, normal+1, best);
        ret %= mod;
    }
    return ret;
}
