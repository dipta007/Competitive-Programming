...
void dfs(int src, int lvl){
    ...
    st[src] = len-1;
    mx = max(mx, lvl);
    for(pii v : adj[src]) dfs(v.ss, lvl+1);
}
void precalc(){
    for(int i = 1; i <= mx; i++){
    ...
    while(cin >> n >> m){
        ...
        while(m--){
            ...
            for(int i = 1; i <= mx; i++){
                ...
                //cout << endl;
                ll temp = value[i].back();
                if(level[root] == i)
                {
                   // debug(root);
                    ll l = st[root];
                    ll r = min(l+c-1, (ll)value[i].size()-1);
                    //debug(l,r);
                    temp -= value[i][r] - value[i][l-1];
                    c *= 2LL;
                    int x = 0, y = 0;
                    if(adj[root].size() == 2){
                        x = adj[root][0].ss, y = adj[root][1].ss;
                        if(st[x] < st[y]) root = x;
                        else root = y;
                    }
                    else if(adj[root].size() == 1) root = adj[root][0].ss;
                }
               // debug(temp,root);
                res = max(res, temp);
            }
