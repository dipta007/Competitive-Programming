...
struct data
{
    int in, last, flg;
    data() {}
    data(int _in, int _last, int _flg)
    {
        in = _in,
        last = _last,
        flg = _flg;
    ...
    while(cin >> st)
    {
        ...
        while(qq--)
        {
            ...
            if(tt == "push")
            {
                ...
                int flg = 0;
                int pos = upper_bound(ALL(v[kk]), last) - v[kk].begin();
                if(pos < v[kk].size()) flg = 1;
                if(q.size() > 0 && q.top().flg == 0) flg = 0;

                if(flg) cout << "YES" << endl;
                else cout << "NO" << endl;

                if(pos < in[kk].size()) last = in[kk][pos];
                else last = st.size();
                q.push(data(kk, last, flg));
            ...
            else
            {
                ...
                if(q.empty()) last = -1;
                else last = q.top().last;
                trace(last);

                if(q.empty() == 1 || q.top().flg == 1)
                {
                    cout << "YES" << endl;
                }
                else cout << "NO" << endl;
