...
void propagate(int node,int b,int e)
{
    ...
    int tot = mid - b + 1;
    tree[Left].sum = tot - tree[Left].sum;
    ///update tree[right].sum
    tot = e - mid - 1 + 1;
    tree[Right].sum = tot - tree[Right].sum;
	///update propagation
...
void update(int node,int b,int e,int i,int j, int newVal=0)
{
	...
	if (b >= i && e <= j)
    {
        tree[node].prop ^= 1;
        int tot = e - b + 1;
        if(tree[node].prop) tree[node].sum = tot - tree[node].sum;
		return;
	}
	int Left=node<<1;
	int Right=(node<<1)+1;
...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    while(~getI(n))
    {
        ...
        FOR(i,1,q)
        {
            ...
            if(st[0] == 'g')
            {
                debug(range[x].ff, range[x].ss);
                int ans = query(1,1,n,range[x].ff, range[x].ss);
                printf("%d\n",ans);
            }
            else
            {
                update(1,1,n,range[x].ff,range[x].ss);
            }
        }
