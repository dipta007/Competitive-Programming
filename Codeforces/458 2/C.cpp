...
ll DP( int pos, int isSmall,int isStart, int value)
{
    ...
    {
        if(value == 0) return 0;
        // debug(value, step[value])
        return step[value] == k-1;
    }
    ...
    if( !isStart )  // আগেই নাম্বার বসানো শুরু করে দিছি
    {
        for ( i = 0 ; i <= ses ; i++ )
        {
            ret += DP( pos + 1, isSmall | i < inp[pos],0, value + i );
            ret %= mod;
        }
    }
    else
...
ll Cal( string st )
{
    ...
    lim = inp.size();
    tt++;
    return DP( 0, 0, 1, 0 )%mod;    // শুধু ০ টা আলাদা এড করছি
}
