...
ll call(int in, int level) {
  if(in >= n) return (level == 0 ? b : (ll)INT_MAX);

  ll &ret = dp[in][level];
  if(ret != -1) return ret;

  ret = LLONG_MAX;
  ll pillar = (level == 0 ? b : b + b);
  if (level == 0 && st[in+1] == '0') {
    ret = min(ret, a + pillar + call(in+1, 0));
    ret = min(ret, a + a + pillar + call(in+1, 1));
  }
  else if (level == 0 && st[in+1] == '1') {
    ret = min(ret, a + a + pillar + call(in+1, 1));
  }
  else if (level == 1 && st[in+1] == '0') {
    if(st[in] == '0') ret = min(ret, a + a + pillar + call(in+1, 0));
    ret = min(ret, a + pillar + call(in+1, 1));
  }
  else if(level == 1 && st[in+1] == '1') {
    ret = min(ret, a + pillar + call(in+1, 1));
  }
