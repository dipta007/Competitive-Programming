...
set <ll> v;
void call(int in, ll now, int tot, ll power) {
  if (tot > 3) return;
  if(in >= 14) {
    if(tot == 3 && v.size() < 100) v.insert(now);
    return;
  }

  call(in+1, now + power, tot+1, power * 2LL);
  call(in+1, now + 0, tot, power * 2LL);
}

set <ll> b;
void call1(int in, ll now, int tot, ll power) {
  if (tot > 11) return;
  if(in >= 14) {
    if(tot == 11) b.insert(now);
    return;
  }

  call1(in+1, now + power, tot+1, power * 2LL);
  call1(in+1, now + 0, tot, power * 2LL);
}
