    ...
    while(cin >> a >> b)
    {
        ...
        for(ll i=1; i<=sq; i++)
        {
            if(tot % i == 0)
            {
                ...
                auto pa = lower_bound(ALL(va), aa) - va.begin();
                if(binary_search(ALL(va), aa) == false) pa--;
                if(pa >= 0 && a/va[pa] <= bb) flg = 1;

                auto pb = lower_bound(ALL(vb), aa) - vb.begin();
                if(binary_search(ALL(vb), aa) == false) pb--;
                if(pb >= 0 && b/vb[pb] <= bb) flg = 1;

                trace(pa, pb, va[pa], vb[pb]);

                if(flg)
                {
                    res = min(res, aa+aa+bb+bb);
                }
                    trace(i, aa, bb, res);
            }
        }
