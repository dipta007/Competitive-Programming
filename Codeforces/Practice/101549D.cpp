...
int main() {
    ...
    while(cin >> n)
    {
        ...
        FOR(i,0,n-1)
        {
            va.push_back(dist(vd[i], a));
            vb.push_back(dist(vd[i], b));
        }

        sort(ALL(va));
        sort(ALL(vb));

        for(auto v: va) trace(v);
        for(auto v: vb) trace(v);
        ...
        FOR(i,1,q)
        {
            ...
            int posa = upper_bound(ALL(va), r1) - va.begin();
            int posb = upper_bound(ALL(vb), r2) - vb.begin();
            trace(posa, posb, n);
            cout << max(0, n -  posa - posb) << endl;
        }
    }
