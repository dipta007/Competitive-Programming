...
int call(int in, int open, int i)
{
    if(open < 0) return 0;
    if(in >= n2)
    {
        return open == 0 && i == st.size();
    }
    ...
    if(i && i<st.size())
    {
        if(st[i] == '(') ret += call(in+1, open+1, i+1);
        else ret += call(in+1, open-1, i+1);
        ret %= MOD;
    }
    else
    {
//        ret += call(in+1, open+1, i);
//
//        ret += call(in+1, open-1, i);
