...
void divisor ( ll n )
{
    ...
    for ( ll i = 0; i < primes.size() && (ll)primes[i] <= sqrtn; i++ )
    {
        if ( n % (ll)primes[i] == 0 )
        {
            ...
            countfactor.push_back((ll)p);
        }
    }
    if ( n != 1 )
    {
        mainfactor.push_back((ll)n);
        countfactor.push_back(1L);
    }
}
...
int main()
{
    ...
    FOR(ci,1,t)
    {
        ...
        divisor(n);
        alldivisor(0, 1);
//        debug(allfactor.size());
        int cnt=0;
        ll nn = sqrt(n);
        sort(ALL(allfactor));
        FOR(i,0,(int)allfactor.size()-1)
        {
            //debug(allfactor[i], nn)
            if(allfactor[i]>nn) break;
            if(allfactor[i]>=d && n/allfactor[i]!=allfactor[i]) cnt++;
        }
        printf("Case %d: %d\n",ci,cnt);
