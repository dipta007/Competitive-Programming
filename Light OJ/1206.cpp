...
{
    int kk = v[x].m + (abs(v[x].x2 - v[x].x1) + abs(v[x].y2 - v[x].y1));
    kk += abs(v[x].x2 - v[y].x1) + abs(v[x].y2 - v[y].y1);
...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    FOR(ci,1,t)
    {
        ...
        {
            int h, m;
            sf("%d:%d %d %d %d %d", &h, &m, &v[i].x1, &v[i].y1, &v[i].x2, &v[i].y2);
            v[i].m = h*60 + m;
        }
        ...
        cout << "Case " << ci << ": " << n - kuhn.match(n) << endl;
    }
