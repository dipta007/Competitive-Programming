...
///****************** template ends here ****************
/// Link: http://www.voidcn.com/article/p-bdoqvupa-pn.html
...
int solve()
{
    ...
    {
        dp[prev][i] = (i<=k ? i : 0);
        sum[prev][i] = (sum[prev][i-1] + dp[prev][i]) % MOD;
        ssum[prev][i] = (ssum[prev][i-1] + sum[prev][i]) % MOD;
    }
    FOR(d,2,n)
    {
        FOR(v, 1, s)
        {
            if(v <= k) dp[curr][v] = ssum[prev][v-1];
            else dp[curr][v] = mod(mod(ssum[prev][v-1] - ssum[prev][v-k-1]) - ((ll)k * sum[prev][v-k-1])%MOD );

            sum[curr][v] = (sum[curr][v-1] + dp[curr][v])%MOD;
            ssum[curr][v] = (ssum[curr][v-1] + sum[curr][v])%MOD;

//            trace(d, v, dp[curr][v], sum[curr][v], ssum[curr][v], prev, curr );
        }
