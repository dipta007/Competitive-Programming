...
point getPointFrom(point p, point q, double m1, double m2)
{
    ...
    res.x = (m1 * q.y + m2 * p.y) / (m1 + m2);
    return res;
}

point get(point p, double a, double b, double c, bool flg)
{
    double s = (a + b + c) / 2.0;
    double A = sqrt(s * (s-a) * (s-b) * (s-c));
    double h = (2.0 * A) / b;
    double theta = asin(h / a);

    int gun = (flg == false) ? 1 : -1;
    if(c*c > b*b + a*a)
    {
        trace("sds", a, b, c);
        gun *= -1;
...
//        READ("in.txt");
    ...
    FOR(ci,1,t)
    {
        ...
        point D = get(A, d, a-c, b, false);
        point C = get(B, b, a-c, d, true);
