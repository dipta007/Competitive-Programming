...
ll call(int small, int big, int inc, int flg)
{
    if(inc == k) flg = 1;
    if(small + big == 0)
    {
    ...
    ll &ret = dp[small][big][inc][flg];
    int &v = vis[small][big][inc][flg];
    if(v == tt) return ret;
    v = tt;
    ...
    if(inc < k)
    {
        FOR(i,1,big)
        {
            ret += call(small+i-1, big-i, inc+1, flg);
            ret %= MOD;
        }
    }
    FOR(i,1,small)
    {
        ret += call(i-1,big+small-i,1, flg);
        ret %= MOD;
    }
    return ret;
}
...
{
    tt++;
    for(int kk=1; kk<=50; kk++, tt++)
    {
        FOR(nn, 1, 50)
        {
            k = kk;
            n = nn;
            ans[nn][k] = call(0, nn, 0, 0);
        }
    }
}
