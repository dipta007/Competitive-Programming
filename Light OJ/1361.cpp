...
int main() {
    ...
    FOR(ci, 1, t)
    {
        int n, m;
        cin >> n >> m;
        FOR(i, 1, n) cin >> topCost[i];
        FOR(i, 1, n) cin >> bottomCost[i];
        FOR(i, 1, n) cin >> topOrBottom[i];

        int top = 0, bottom = n+1;
        flow::init(n+7, top, bottom);
        ...
        FOR(i,1,n)
        {
            int tc = topCost[i];
            int bc = bottomCost[i];
            if(topOrBottom[i] == TOP) bc = INF;
            if(topOrBottom[i] == BOTTOM) tc = INF;

            flow::addEdge(top, i, tc);
            flow::addEdge(i, bottom, bc);
        }
