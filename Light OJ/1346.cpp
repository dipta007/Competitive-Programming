...
int main() {
    ...
    FOR(ci,1,t)
    {
        ...
        FOR(i,1,n-1)
        {
            ...
            if(i > 1)
            {
                //trace(i, p.x, p.y, l.a.x, l.a.y, l.b.x, l.b.y, turn(l.a, l.b, p));
                if(turn(l.a, l.b, p) == 1)
                {
                    point intersect;
                    intersect = l.crossPoint(line(prev, p));
                    res += intersect.distance(p);
                    trace(i, res, intersect.x, intersect.y, l.a.x, l.a.y, l.b.x, l.b.y, prev.x, prev.y, p.x, p.y);
                    l = line(aladin, p);
                }
            }
            else
            {
                if(p.y != prev.y)res += prev.distance(p);
                l = line(aladin, p);
                trace(prev.x, prev.y, p.x, p.y);
                trace(i, res);
            }
            prev = p;
        }
