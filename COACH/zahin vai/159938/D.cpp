...
//        debug("l",kk);
    ...
    while(~getI(n))
    {
        ...
        int flg=0;
        int a,b;
        int x = lower_bound(ALL(lucky), n/2) - lucky.begin();
        if(binary_search(ALL(lucky), n/2)==false) x--;
        debug(lucky[x])
        ROF(i,x,0)
        {
            //if(lucky[i] > n/2) break;
            int kk = n-lucky[i];
            if(binary_search(ALL(lucky) , kk)==true)
            {
                flg=1;
                a=lucky[i];
                b=n-lucky[i];
        ...
        if(a>b) swap(a,b);
        if(flg) printf("%d is the sum of %d and %d.\n",n,a,b);
