...
public:
    ...
    ll getForwardHashingQuery(int left, int right)
    {
        if(left==0)
        {
            debug(forwardHashTable[0][right],forwardHashTable[1][right]);
    ...
    ll getForwardHashingQuery2(int left, int right)
    {
        if(left==0)
        {
            debug(forwardHashTable[0][right],forwardHashTable[1][right]);
            return forwardHashTable[1][right];
        }
        ll ret2 = MOD(forwardHashTable[1][right] - (forwardHashTable[1][left-1] * power[1][right-left+1])%mod[1] , 1);
    ...
    ll getBackwardHashingQuery1(int left, int right)
    {
        ll ret1 = MOD(backwardHashTable[0][left] - (backwardHashTable[0][right+1] * power[0][right-left+1])%mod[0] , 0);
        ll ret = (ret1 );
        return ret;
    }
    ll getBackwardHashingQuery2(int left, int right)
    {
        ll ret2 = MOD(backwardHashTable[1][left] - (backwardHashTable[1][right+1] * power[1][right-left+1])%mod[1] , 1);
        ll ret = ret2;
        return ret;
    ...
    {
        ll lftFor1 = getForwardHashingQuery1(0,x-1);
        ll lftFor2 = getForwardHashingQuery2(0,x-1);
        ll rgtFor1 = getForwardHashingQuery1(x+1, sz);
        ll rgtFor2 = getForwardHashingQuery2(x+1, sz);
        ll totFor1 = ((lftFor1 * power[0][sz-x])%mod[0] + rgtFor1)%mod[0];
        ll totFor2 = ((lftFor2 * power[1][sz-x])%mod[1] + rgtFor2)%mod[1];
        ll totFor = (totFor1 << 31) | totFor2;

        ll lftBack1 = getBackwardHashingQuery1(0,x-1);
        ll lftBack2 = getBackwardHashingQuery2(0,x-1);
        ll rgtBack1 = getBackwardHashingQuery1(x+1, sz);
        ll rgtBack2 = getBackwardHashingQuery2(x+1, sz);
//        ll totBack1 = ((lftBack1 * power[0][sz-x])%mod[0] + rgtBack1)%mod[0];
//        ll totBack2 = ((lftBack2 * power[1][sz-x])%mod[1] + rgtBack2)%mod[1];
        ll totBack1 = (lftBack1 + (rgtBack1 * power[0][x])%mod[0])%mod[0];
        ll totBack2 = (lftBack2 + (rgtBack2 * power[1][x])%mod[1])%mod[1];
        ll totBack = (totBack1 << 31) | totBack2;

        debug(x,totFor, totBack)
        return totFor == totBack;
    }
};
...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    while(getline(cin,st))
    {
        ...
        {
            if(helper.check(i, (int)st.size()-1))
            {
                printf("YES\n%d\n",i+1);
                flg=1;
                break;
        ...
        helper.makeForwardHashTable("eertree");
        helper.makeBackwardHashTable("eertree");
        debug("*********************")
        debug(helper.getForwardHashingQuery(0,0))
        debug(helper.getForwardHashingQuery(1,6))
        debug(helper.getForwardHashingQuery(0,6))
//        debug(helper.getBackwardHashingQuery(0,6))
        if(flg==0)
        {
            printf("NO\n");
        }
