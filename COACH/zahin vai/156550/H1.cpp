...
//        }
        ...
        FOR(i,1,n)
        {
            ...
            FOR(j,0,sz-1)
            {
                ...
                getI(x);
                v.PB(x);
//                mvv[num][szOfVec[num]].PB(x);
            }
            mvv[num].PB(v);
            index[ pii(num, szOfVec[num]) ] = i;
            szOfVec[num]++;
        }
        map <int, int> kajSesh;
        map <int, int> res;
        ROF(i,100,0)
        {
            ...
            FOR(j,0,szOfVec[i]-1)
            {
                int szz = mvv[i][j].size();
                //debug(szz)
                FOR(k,0,szz-1)
                {
                    //debug(mvv[i][j][k],index[pii(i,j)])
                    pq.push(MP( pii(k, index[pii(i,j)]), mvv[i][j][k] ));
                }
            }
            ...
            while(!pq.empty())
            {
                ...
                if(mp.find(p.ss) > 0 && kajSesh[p.ff.ss]==0)
                {
                    kajSesh[p.ff.ss] = 1;
                    mp.add(p.ss, -1);
                    res[p.ff.ss] = p.ss;
