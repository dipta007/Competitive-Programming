...
{
public:
    unsigned int a[100][100];   ///Maximum dimension of the matrix, Set Long Long
    int row, col;
    MAT() {}
    MAT(int r, int c)
    {
    ...
    FOR(a,0,9)
    {
        FOR(b,a+1,9)
        {
            ...
            FOR(c,0,9)
            {
                FOR(d,c+1,9)
                {
                    if(d==c+1 || d==c || d==c-1) continue;
                    if(c-1 == a || c==a || c+1==a || c-1==b || c==b || c+1==b) continue;
                    if(d-1 == a || d==a || d+1==a || d-1==b || d==b || d+1==b) continue;
