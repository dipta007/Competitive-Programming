...
void call(int in)
{
    ...
    FOR(i,in,mlen-1)
    {
        ll now = helper.getForwardHashingQuery(in, i);
        debug(i,in+i-1, now, ase[now])
        if(ase[now]==1)
        {
            vs.PB(rev[now]);
            call(i+1);
...
int main() {
    ...
    while(cin >> mlen)
    {
        ...
        FOR(i,0,n-1)
        {
            ...
            cin >> st;
            string stt = st;
            FOR(j,0,(int)st.size()-1) st[j] = tolower(st[j]);
            ll now = helper.getBack(st);;
            debug(st, now);
            rev[now] = stt;
            ase[now] = 1;
        }
