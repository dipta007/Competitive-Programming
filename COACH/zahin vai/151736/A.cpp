...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    while(~getI(n))
    {
        ...
        while(1)
        {
            ...
            if(d.size()==n+1)
            {
                ll now = MOD(d.back()-d.front());
                ll vag = bigmod(pp, in.front()+1, mod);
                ll mvag = modularInverse(vag, mod);
                ll res = (now%mod * mvag)%mod;
                //debug(d.back(), d.front(), now, mvag, res,in.front()-1)
                d.pop_front();
                in.pop_front();
                if(res==pat)
                {
