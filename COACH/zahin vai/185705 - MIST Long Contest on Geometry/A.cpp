...
//    ios_base::sync_with_stdio(0);cin.tie(0);
    ...
    while(~getI(n))
    {
        ...
        vector <line> vl;
        vl.PB( line( point(x1, y2) , point(x1,y1) ));
        vl.PB( line( point(x2, y2) , point(x2,y1) ));

        FOR(i,1,n)
        {
            int x,y;
            getII(x,y);
            vl.PB( line( point(y,y2) , point(x,y1) ));
        }

        sort(ALL(vl), comp);

        map <int, int> mp, koita;
        FOR(i,1,m)
        {
            int x,y;
            getII(x,y);
            ...
            FOR(j,0,(int)vl.size()-2)
            {
                int a = vl[j].relation(p);
                int b = vl[j+1].relation(p);
...
//                debug(i, j, a, b)
        ...
        printf("Box\n");
        for(it=koita.begin(); it!=koita.end(); it++)
        {
            printf("%d: %d\n",it->ff,it->ss);
        }
    }
