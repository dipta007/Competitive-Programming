...
//    }
    ...
    while(~getII(m,n))
    {
        ...
        if(m==0 && n==0 && src==0 && sink==0) break;

        debug(m,n,src,sink)

        mf::init(0, n+1, n+4);

        mf::addEdge(0,src, 1000,0);
        mf::addEdge(sink, n+1, INF,0);
        ...
        FOR(i,1,m)
        {
            ...
            getF(dw);
            if(dw + EPS< -6.0) w = 0;
            else if(dw > -7+EPS && dw+EPS < 8)
            {
                dw = exp(dw);
                dw *= 1000.0;
                w = (int)(dw);
            }
            else if(dw > 7+EPS && dw+EPS<711)
            {
                w = 1000;
            }
            else w = 100000;
