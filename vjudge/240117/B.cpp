...
namespace nodeflow
{
    void init(int n, int source, int sink, vi capacity)
    {
        //think about the sink, it can be sink * 2
        flow::init(n * 2, source * 2, sink * 2 + 1);
        flow::addEdge(source*2, source*2+1, 2);
        flow::addEdge(sink*2, sink*2+1, INF);
        for (int i = 1; i < n-1; i++)
        {
            trace(i, capacity[i-1]);
            flow::addEdge(i * 2, i * 2 + 1, capacity[i-1]);
        }
    }
...
int main() {
    ...
    FOR(ci, 1, t)
    {
        ...
        while(low <= high)
        {
            ...
            nodeflow::init(n+2, 0, n+1, cap);
            if(d <= mid)
            {
                nodeflow::addEdge(0, n+1, INF);
//                trace(0, n+1);
            }
            FOR(i,0,n-1)
            {
//                trace(i, v[i], mid, d-v[i], d);
                if(mid >= v[i])
                {
//                    trace(0, i+1);
                    nodeflow::addEdge(0, i+1, INF);
                }
                if(d-v[i] <= mid)
                {
//                    trace(i+1, n+1);
                    nodeflow::addEdge(i+1, n+1, INF);
                }
            }
//
            FOR(i,0,n-1)
            {
                FOR(j,i+1, n-1)
                {
                    ...
                    {
                        trace(i,j);
                        nodeflow::addEdge(i+1, j+1, INF);
                    }
                }
            }

            int now = nodeflow::dinic();
            trace(low, mid, high, now);
            if(now == 2)
            {
                res = mid;
                high = mid - 1;
