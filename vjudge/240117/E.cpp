...
namespace nodeflow
{
    void init(int n, int source, int sink)
    {
        ...
        flow::addEdge(source*2, source*2+1, INF);
...
//            trace(st[i], n, m, tot);
            FOR(j,0,m-1)
            {
                ...
                {
                    nodeflow::addEdge(i*m+j+1, sink, INF);
                }
                if(st[i][j] == '*')
                {
                    tot++;
                    nodeflow::addEdge(src, i*m+j+1, INF);
                }
            }
        }
        ...
        FOR(i,0,n-1)
        {
            FOR(j,0,m-1)
            {
                FOR(k,0,3)
                {
                    ...
                    if(x >= 0 && y >= 0 && x<n && y<m)
                    {
                        nodeflow::addEdge(i*m+j+1, x*m+y+1, INF);
                    }
                }
            }
        }

        int res = nodeflow::dinic();
        trace(res, tot);
        cout << "Case " << ci << ": ";
        if(res == tot) cout << "yes" << endl;
        else cout << "no" << endl;
    }
