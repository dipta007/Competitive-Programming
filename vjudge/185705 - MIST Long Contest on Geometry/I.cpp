...
{
    double A = fabs(triArea2(a,b,c)) + fabs(triArea2(a,c,d));
...
//            debug(c[i])
        ...
        {
            vector <point>  rgt;
            line ab( point(a[i], 0), point(b[i],1) );

//            debug(i, a[i], b[i])

            FOR(j,0,(int)c.size()-1)
            {
                point cp = ab.crosspoint( line( point(0, c[j]), point(1,d[j]) ) );
                rgt.PB(cp);
            }

            FOR(j,0,(int)lft.size()-2)
            {
                double now = area(lft[j], rgt[j], rgt[j+1], lft[j+1]);

                if(now > maxy + EPS) maxy = now;
            }
