...
int BFS2d(point src,point dest)
{
    ...
    while(!q.empty())
    {
        ...
        if(u.x==dest.x && u.y==dest.y) return dist[dest.x][dest.y];
        int cnt=0;
...
//                cnt++;
                visited[X][Y]=1;
                dist[X][Y] = dist[u.x][u.y]+mdst[X][Y];
                q.push(point(X,Y));
            }
        }
        //debug(u.x, u.y, cnt)
