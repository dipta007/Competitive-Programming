...
int cnt[A], a[N], freq[A], ans[N], answer = 0;
...
void add(int position) {
    if(freq[a[position]] != 0) cnt[freq[a[position]]]--;
    freq[a[position]]++;
	cnt[freq[a[position]]]++;
    answer = max(answer, freq[ a[position] ]);
}

void remove(int position) {
    cnt[freq[a[position]]]--;

	if(freq[a[position]] == answer && cnt[ freq[ a[position] ] ] == 0) {
