...
int main() {
    ...
    FOR(ci,1,t)
    {
        ...
        cin >> m >> n;
        int tot = 0;
        FOR(i,1,m) cin >> sponsor[i];
        FOR(i,1,m) tot += sponsor[i];
        FOR(i,1,n) cin >> instrument[i];

        int src = 0, sink = m+n+1;
        flow::init(sink+1, src, sink);

        FOR(i,1,m) flow::addEdge(src, n+i, sponsor[i]);
        FOR(i,1,m)
        {
            int q;
            cin >> q;
        ...
        FOR(i,1,n) flow::addEdge(i, sink, instrument[i]);

        cout << "Case " << ci << ": " << tot - flow::dinic() << endl;
    }
