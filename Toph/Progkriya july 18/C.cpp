...
double dist(piii a, piii b)
{
    return sqrt(sq(a.ff - b.ff) + sq(a.ss.ff - b.ss.ff) + sq(a.ss.ss - b.ss.ss));
}

double getArea(point p1, point p2, int h)
{
    double a = dist(MP(p1.x, MP(p1.y, 0)), MP(p2.x, MP(p2.y, 0)));
    double b = dist(MP(0, MP(0, h)), MP(p2.x, MP(p2.y, 0)));
    double c = dist(MP(p1.x, MP(p1.y, 0)), MP(0, MP(0, h)));
...
int main() {
    ...
    FOR(ci,1,t)
    {
        ...
        double B = poly.getArea();
        double vol = (B * h)/3.0;
        double SA = 0.0;
        FOR(i,0,n-1)
        {
            SA += getArea(poly.p[i], poly.p[(i+1)%n], h);
        }
