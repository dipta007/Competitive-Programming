...
double call(int in, int make)
{
    double &ret = dp[in][make];
    if(in == 0) return (make==0);
    if(vis[in][make] == 1) return ret;
    vis[in][make] = 1;

    ret = 0.5 * call(in-1, make-1) + 0.5 * call(in-1, make);
    return ret;
}
