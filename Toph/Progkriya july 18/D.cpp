...
int main() {
    ...
    for(ll i=2;i<=33;i++)
    {
        ca[i]=(ca[i-1]*(4LL*i+2LL))/(i+2LL);
        cc.PB(ca[i]);
//        trace(ca[i], i);
    }
    FOR(i,0,(int)cc.size()-1)
        trace(i, ca[i], ca[i] <= 1e18);
    ull mc = 285894211472315003LL;
    FOR(ci,1,t)
    {
        ll n;
        cin >> n;
        ...
        int kk = lower_bound(ALL(cc), n) - cc.begin();
        if(binary_search(ALL(cc), n) == false) kk--;
        cout << kk << endl;
    }
