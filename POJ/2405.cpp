...
double getConeVolume(double h, double R, double r)
{
    return (PI * h * (R * R + R * r + r * r))/3.0;
}

double get(double d, double D)
{
    double R = D/2.0;
    double total = getCylinderVolume(R, D);
    double baki = getCylinderVolume(d/2.0, d) + 2.0 * getConeVolume((D-d)/2.0, D/2.0, d/2.0);
