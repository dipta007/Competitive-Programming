...
ll mod = 1000000007;
ll call(vi &v, int len) {
    if(len == 0) return v[0] % mod;
    return (call(v, len-1) * 100001 + v[len])%mod;
}
...
int main() {
    ...
    while(cin >> s)
    {
        ...
        FOR(i,1,q)
        {
            ...
            vi res = multiply(v, v1);

            vi hd;
            for(int i=v1.size()-1, k=1; k<=v.size()-v1.size()+1; k++, i++)
            {
                int x = res[i];
                int y = v1.size();
                hd.PB((y-x)/2);
