...
double length2(Point a)    // find the length of a vector, squared
{
    return dot(a,a);
}

double length(Point a)     // find the length of a vector
{
    return sqrt(dot(a,a));
}
...
int plane_iline_isect(Point norm, Point ori, Point a, Point b, Point *p)
{
    ...
    if (fabs(den) < EPS)   /* parallel */
    {
        if (length2(cross(ori-a,b-a)) < EPS)
            return -1; /* coincident */
