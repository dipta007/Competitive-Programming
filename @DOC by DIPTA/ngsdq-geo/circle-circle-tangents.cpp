...
int getTangents(circle A,circle B,vector<line> &v){
	...
	double rsum = A.r+B.r;
	if(dblcmp(d2-rdiff*rdiff)<0) return 0;
	point va = B.p-A.p;
	double base = atan2(va.y,va.x);
	if(dblcmp(d2)==0&&dblcmp(A.r-B.r)==0) return -1;
	if(dblcmp(d2-rdiff*rdiff)==0){
	...
	a[cnt] = getPoint(A,base-ang);b[cnt++] = getPoint(B,base-ang);
	if(dblcmp(d2-rsum*rsum)==0){
		a[cnt] = getPoint(A,base);b[cnt++] = getPoint(B,PI+base);
	}else if(dblcmp(d2-rsum*rsum)>0){
		double ang = acos((A.r+B.r)/sqrt(d2));
