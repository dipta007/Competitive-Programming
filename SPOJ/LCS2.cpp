...
//    FOR(i,0,strlen(sa.str)-1)
//    {
//        trace(i, sa.sa[i], sa.lcp[i]);
//    }
    ...
    FOR(i,1,strt - 2)
    {
        unordered_set <int> s;
        int mn = INF;
        s.insert(arr[ sa.sa[i+1] ]);
        for(int j=i, k=1; j>0 && k < c; j--, k++)
        {
            s.insert(arr[sa.sa[j]]);
            mn = min(mn, sa.lcp[j]);
        }

//        trace(i, sa.sa[i], sa.lcp[i], s.size(), mn);
