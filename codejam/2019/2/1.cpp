...
int main()
{
    ...
    FOR(ci,1,t) {
      ...
      FOR(step, 0, 499) {
        set < char > s;
        FOR(j,0,(int)vs.size()-1) {
          if(mark[j]) continue;

          int pos = step % (int)vs[j].size();
          s.insert(vs[j][pos]);
        }

        // cout << s.size() << endl;
        // for(auto vv: s) cout << vv << endl;
