...
bool isNeeded(int pos, set<char> &s, map <int, bool> &series) {
  if(series[pos]) return false;

  set <char> thakte_pare;
  thakte_pare.insert('A');
  thakte_pare.insert('B');
  thakte_pare.insert('C');
  thakte_pare.insert('D');
  thakte_pare.insert('E');

  for(auto v: fig[pos]) thakte_pare.erase(v);

  for(auto v: s) if(thakte_pare.count(v)) return true;
    ...
    FOR(ci,1,t) {
      ...
      FOR(i,1,4) {
        ...
        if(s.size() > 1) {
          FOR(j,1,119) {
            int rnd = rand() % 119 + 1;
            while(isNeeded(rnd, s, series) == false) rnd = rand() % 119 + 1;
            ...
            if(mp[st[0]] == 24 && s.count(st[0])) s.erase(st[0]);
