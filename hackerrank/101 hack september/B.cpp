...
int dfs(int u,int prev)
{
    ...
    {
        int v = AdjList[u][j];
        if(dfs_num[v]==DFS_BLACK) continue;
        if(prev==0)
        {
            if(mark[v]) cnt += (dfs(v,1));
            else cnt+= (1+dfs(v,1));
        }
        else
        {
...
//
//int call(int in,int prevF)
//{
//    if(in>n) return 0;
//    if(dp[in][prevF]!=-1) ret urn dp[in][prevF];
//
//    int s1,s2;
//    s1=s2=INF;
//
...
int main()
{
    ...
    for(int ci=1; ci<=t; ci++)
    {
        ...
        for(int i=1;i<=n;i++)
        {
            if(inOut[i]!=1) continue;
            dfs_num.assign(n+4, DFS_WHITE);
            if(mark[i]) mini=min(mini,dfs(i,1));
            else mini=min(mini,dfs(i,0));
            //debug(mini);
        }
        printf("%d\n",mini);
    }
